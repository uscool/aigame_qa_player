## AI Game QA tester

This project is a failed attempt to produce an artificial intelligence system from scratch that takes user input from a single play through of a game, and then explores it using some changes. It was deemed too complex for phased time frame, although doable. It has been tested on SpaceWalk, a respository from Angel1841

Credits: https://github.com/Angel1841/Space-Walk